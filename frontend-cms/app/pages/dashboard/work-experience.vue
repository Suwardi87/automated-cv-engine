      ...
      <div class="relative z-10 w-full max-w-lg rounded-2xl border border-zinc-800/60 bg-zinc-900 shadow-2xl">
        ...
        <div class="space-y-5 px-6 py-6">
          ...
          <div>
            ...
            <RichTextEditor v-model="form.description" placeholder="Deskripsikan tanggung jawab dan pencapaian Anda..." />
          </div>

          <div>
            <div class="mb-1.5 flex items-center justify-between">
              <label class="block text-xs font-medium text-zinc-400">Poin Penting</label>
              <button
                type="button"
            ...
            <RichTextEditor v-model="form.highlights" placeholder="Tulis poin-poin penting (satu per baris)" />
          </div>
        </div>
...
async function handleImprove(field: string) {
  ...
  try {
    ...
    const res = await api.post<{ success: boolean; data: { improved: string } }>('/ai/improve', {
      text,
      context: field === 'description' ? 'deskripsi pengalaman kerja profesional' : 'pencapaian dan kontribusi di pekerjaan',
    })
